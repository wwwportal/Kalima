...
use kalima_core::{Annotation, EngineError, QuerySpec, SearchBackend, SegmentView, StorageBackend};
use kalima_index::TantivyIndex;
use kalima_store::{ConnectionRecord, SqliteStorage, SurahSummary};
use std::sync::Arc;
...
async fn segment_handler(
    ...
    match state.storage.get_segment(&id).await.map_err(map_err)? {
        ...
        None => Err(map_err(kalima_core::EngineError::NotFound)),
    }
}
...
async fn seed_demo(storage: &SqliteStorage, search: &TantivyIndex) -> Result<(), (StatusCode, String)> {
    let doc = SegmentView {
        ...
        token_index: 0,
        text: "بِسْمِ".into(),
        segments: vec![kalima_core::Segment {
            id: "seg-1".into(),
            r#type: "prefix".into(),
            form: "بِ".into(),
            root: None,
...
async fn create_annotation_verse(
    ...
    let ann = kalima_core::Annotation {
